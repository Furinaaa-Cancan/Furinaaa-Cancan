<div align="center">

# Cancan

**Biomedical Engineering (BME)** · Agentic AI Systems · Clinical Prediction Modeling

</div>

---

### Engineering

**Agentic RAG · code/repo search (ripgrep) · LLM / Agent** — architecture design and implementation of agent systems.

**Full-stack development + deployment & ops** — delivering end-to-end enterprise systems.

<details>
<summary><b>中文简介</b></summary>

<br/>

Agentic RAG · 代码/仓库检索 (ripgrep) · LLM / Agent —— 智能体架构设计与落地。

全栈开发 + 部署运维，交付端到端企业级系统。

</details>

---

### Bioinformatics

**Heterogeneity-driven association analysis** across large population-cohort and dietary-composition databases — **NHANES · KNHANES · UK Biobank · CHARLS · ELSA · ELSA-Brasil · USDA · FNDDS · Phenol-Explorer** — combining machine learning with epidemiological modeling.

Building a **governance framework for AI-driven clinical prediction models** under top-journal review standards.

<details>
<summary><b>中文简介</b></summary>

<br/>

基于大型人群队列与膳食成分数据库（**NHANES、KNHANES、UK Biobank、CHARLS、ELSA、ELSA-Brasil；USDA、FNDDS、Phenol-Explorer**），结合机器学习与流行病学建模，开展异质性（heterogeneity）驱动的关联分析。

建立「顶刊级审稿标准 × AI 驱动」的医学预测模型治理框架。

</details>

---

<div align="center">

<!-- Stats + Streak -->
<img width="46%" src="https://github-readme-stats-sigma-five.vercel.app/api?username=Furinaaa-Cancan&show_icons=true&count_private=true&include_all_commits=true&hide_border=true&bg_color=1F1815&title_color=E8B4A0&icon_color=E8B4A0&text_color=F5E6D8&ring_color=C9826F" />
<img width="46%" src="https://github-readme-streak-stats.herokuapp.com?user=Furinaaa-Cancan&hide_border=true&background=1F1815&stroke=C9826F&ring=E8B4A0&fire=C9826F&currStreakNum=F5E6D8&sideNums=F5E6D8&currStreakLabel=E8B4A0&sideLabels=E8B4A0&dates=A89080" />

<br/><br/>

<!-- Activity Graph 趋势图 -->
<img width="94%" src="https://github-readme-activity-graph.vercel.app/graph?username=Furinaaa-Cancan&hide_border=true&bg_color=1F1815&color=F5E6D8&line=E8B4A0&point=C9826F&area=true&area_color=E8B4A0&custom_title=Contribution%20Activity%20%C2%B7%20%E8%B4%A1%E7%8C%AE%E8%B6%8B%E5%8A%BF" />

</div>
